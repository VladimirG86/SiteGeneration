...
const state = {
  step: -1, // -1 = welcome
  kind: 'landing', // landing | vcard (taplink убран — QR-визитка покрывает мультиссылку)
...
function restoreState() {
  try {
    ...
    if (d.kind) state.kind = d.kind === 'taplink' ? 'vcard' : d.kind;
...
function bindButtons() {
  ...
  const cv = $('#btn-canvas'); if (cv) cv.addEventListener('click', () => startCanvas());
...
async function startCanvas(){
  // free canvas: create immediately without wizard (or via generate with kind=canvas)
  const name = prompt('Название канваса (бренд на первом экране)?', 'Мой канвас') || 'Nelvi Canvas';
  const about = prompt('Коротко о проекте (не обязательно):', '') || '';
  try{
    const r=await fetch('/api/generate', {method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({name, about, products:'', advantages:'', extras:'', theme_mode: state.theme.mode, accent: state.theme.accent, kind:'canvas'})});
    const d=await r.json();
    if(!r.ok) throw new Error(d.error||r.status);
    const id=d.job_id;
    // redirect to canvas editor with id
    location.href='/canvas?id='+encodeURIComponent(id);
  }catch(e){
    alert('Не удалось создать канвас: '+e.message+'\nПробую альтернативный путь…');
    try{
      const r2=await fetch('/api/canvases', {method:'POST'});
      const d2=await r2.json();
      if(r2.ok && d2.id) location.href=d2.url;
      else throw new Error(d2.error||'no id');
...
function renderStep() {
  ...
  // динамические заголовки для vcard (QR-визитка покрывает мультиссылку)
  ...
  if (state.kind !== 'landing') {
    ...
    badge.textContent = 'QR-визитка — myqrcards · ссылки внутри';
  ...
  if (def.id === 'about') {
    ...
    const aboutPh = state.kind === 'landing' ? 'Например: кофейня в центре Москвы. Работаем с 2019 года, своя обжарка, завтраки и кофе с собой.' : 'Например: Иван Петров — дизайнер в Студии Свет, делаю сайты и брендинг.';
  ...
  if (def.id === 'products' || def.id === 'advantages' || def.id === 'extras') {
    ...
    if (state.kind === 'vcard') {
      if (key === 'products') { label = 'Ссылки и услуги (каждая с новой строки)'; ph = 'Портфолио — https://example.com\nЗапись — https://t.me/example\nДизайн сайтов и брендинг'; }
      if (key === 'advantages') { label = 'Телефон / email / соцсети'; ph = '+7 900 123-45-67\nhello@example.ru\nhttps://t.me/example'; }
      if (key === 'extras') { label = 'Дополнительно'; ph = 'Адрес, часы, пожелания по стилю — а также заметка под QR'; }
    }
...
async function beginGeneration() {
  ...
  const poll = setInterval(async () => {
    ...
    if (job.status === 'done') {
      clearInterval(poll);
      if (job.warning) { $('#gen-warning').textContent = job.warning; $('#gen-warning').classList.remove('hidden'); }
      // canvas kind -> сразу в редактор канваса
      const kind = (job.kind || job.site_kind || '');
      if (kind === 'canvas') {
        setTimeout(() => { location.href='/canvas?id='+encodeURIComponent(jobId); }, job.warning ? 1400 : 500);
        return;
      }
