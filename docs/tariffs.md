...
| **Business** | **4 990 ₽** | **39 900 ₽** (3 325/мес) | ∞ | 2 000 | `*.nelvi.app` | ✅ (∞) | ZIP + S3 + WP + API | Команда 5 чел, вебхуки, кастомный CNAME, SLA 99.9%, a11y |
...
- Под карточками — `Год -20%` тоггл `#yearly-toggle` (меняет `amount` на годовой, `yearly:true` → 19 900/39 900, `checkout_url(...yearly)` → `/#prices?tariff=pro&yearly=1`)
